...
export function KitchenStream({ initialOrders }: { initialOrders: Order[] }) {
        ...
        const connect = () => {
            ...
            eventSource.onmessage = (event) => {
                try {
                    ...
                    // Optional: You could do deep equality checks here if you want to prevent re-renders,
                    // but React state updates will naturally batch. We'll simply set the new state.
    ...
    if (orders.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-20 text-center border border-dashed border-zinc-800 rounded-2xl bg-zinc-900/20">
                ...
                <div className={`mt-8 px-3 py-1 text-xs rounded-full inline-flex items-center gap-2 border ${sseStatus === 'connected' ? 'bg-lime-500/10 text-lime-400 border-lime-500/20' : sseStatus === 'connecting' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                    <span className={`w-2 h-2 rounded-full ${sseStatus === 'connected' ? 'bg-lime-400' : sseStatus === 'connecting' ? 'bg-yellow-400 animate-pulse' : 'bg-red-500'}`} />
                    {sseStatus === 'connected' ? 'SSE Aktywne' : sseStatus === 'connecting' ? 'Łączenie...' : 'Błąd Połączenia'}
                </div>
            </div>
        );
    }
    ...
    return (
        <div>
            <div className={`mb-6 px-3 py-1.5 text-xs rounded-full inline-flex items-center gap-2 border ${sseStatus === 'connected' ? 'bg-lime-500/10 text-lime-400 border-lime-500/20' : sseStatus === 'connecting' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
